...
export const SetCoinAds = ({ id = getItem('uid'), num = '0', ads = getItem('myaddress'), otype = getItem('netType') }) => {
...
// 登录 注册 api
// export const RegEotc = ({ userid, pwd, yzm, pid, sign, otype = localStorage.getItem('netType') }) => {
// 	const data = {
// 		userid,
// 		pwd,
// 		yzm,
...
// 	});
// };
//mail（邮箱）、、password（密码)、code（验证码）、refUserId（邀请码）

//基本信息验证
export const SetUID = ({ username, uid = localStorage.getItem('uid'), myzd, ccid, sign, ads = '' }) => {
...
//审核数据获取、通过、打回
export const EotcKyc = ({ pid = localStorage.getItem('uid'), sign = localStorage.getItem('mysign'), uid = 0, kyc = 0, msg = '' }) => {
...
//释放EOTC
export const Get_MyEOTC = ({ uid = localStorage.getItem('uid'), sign = localStorage.getItem('mysign') }) => {
...
//加速
export const myeotc_js = ({ uid = localStorage.getItem('uid'), sign = localStorage.getItem('mysign') }) => {
  const params = { uid, sign };
  return request({
    method: 'POST',
...
//获取团队人员列表
export const GetMyTeams = ({ uid = localStorage.getItem('uid'), sign = localStorage.getItem('mysign') }) => {
  const params = { uid, sign };
  return request({
    method: 'POST',
...
//查看钱包
export const MemberWallet = ({ uid, net = localStorage.getItem('netType') }) => {
...
//获取和设置自身电报群
export const SetTelegram = ({ uid = localStorage.getItem('uid'), sign = localStorage.getItem('mysign'), telegram = '' }) => {
...
export const StakingEotc = ({ ads = localStorage.getItem('myaddress'), sign = localStorage.getItem('mysign'), net = localStorage.getItem('netType'), num, zq = 0 }) => {
  const params = { ads, sign, net, num, zq };
...
export const addOrder = ({ cny, num, amount1, amount2, cash = 0, did = 0, type = 1 }) => {
  const params = {
    cny,
    num,
    amount1,
...
export const UPdateOrder_sj = ({ cny = 0, num, amount1 = 0, amount2 = 0, did, type = 1 }) => {
  const params = {
    cny,
    num,
    amount1,
 ...
 */
export const subbuysellorder = ({ oid, cny = 0, mail = '', selectpayk, type }) => {
  const params = {
    uid: getItem('uid'),
    oid,
    cny,
 ...
 */
export const UpdateOrderType = ({ uid = localStorage.getItem('uid'), did, type = 11, mail, busPay = '' }) => {
  const params = {
    uid,
    did,
    type,
